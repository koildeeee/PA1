...
public:
  ...
  ImgNode(const ImgNode &ndata)
  {
    ...
    south = NULL;
    west = NULL;
    skipright = ndata.skipright;
    skipleft = ndata.skipleft;
    skipup = ndata.skipup;
    skipdown = ndata.skipdown;
  };
};
...
public:
  ...
  *********************************************************/
private:
  void CreateFirstRow(ImgNode *curr, int rowNum, unsigned width, PNG &img);
  ImgNode *CreateRow(ImgNode *curr, int rowNum, unsigned width, PNG &img);
  void RenderImage(ImgNode *init, unsigned width, unsigned height, PNG& outpng) const;
  double GetSmallerHue(double h1, double h2) const;
  int Max(int a, int b);
  void IncSkipEW(ImgNode* toRemove, ImgNode* w, ImgNode* e);
  void IncSkipNS(ImgNode* toRemove, ImgNode* n, ImgNode* s);
};
